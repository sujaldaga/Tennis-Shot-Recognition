...
class ShotCounter:
    ...
    MIN_FRAMES_BETWEEN_SHOTS = 20  # was 60 — too long at low fps
    ...
    def update(self, probs, frame_id):
        ...
        # Debug: print probabilities every 30 frames
        if frame_id % 30 == 0:
            print(
                f"[Frame {frame_id}] B={probs[0]:.2f} F={probs[1]:.2f} "
                f"N={probs[2]:.2f} S={probs[3] if len(probs)>3 else 0:.2f}"
            )
        ...
        if (
            ...
            print(f"  >>> SHOT DETECTED: backhand (prob={probs[0]:.2f})")
        elif (
            probs[1] > 0.70
            and self.frames_since_last_shot > self.MIN_FRAMES_BETWEEN_SHOTS
        ):
            ...
            self.frames_since_last_shot = 0
            self.results.append({"FrameID": frame_id, "Shot": self.last_shot})
            print(f"  >>> SHOT DETECTED: forehand (prob={probs[1]:.2f})")
        elif (
            len(probs) > 3
            and probs[3] > 0.70
            and self.frames_since_last_shot > self.MIN_FRAMES_BETWEEN_SHOTS
            ...
            self.frames_since_last_shot = 0
            self.results.append({"FrameID": frame_id, "Shot": self.last_shot})
            print(f"  >>> SHOT DETECTED: serve (prob={probs[3]:.2f})")
...
if __name__ == "__main__":
    ...
    # Load model with compatibility fix for newer Keras/TF versions
    try:
        m1 = keras.models.load_model(args.model)
        print("Model loaded successfully.")
    except Exception as e:
        print(f"Direct load failed ({e}). Using compatibility mode...")
        # Rebuild the exact model architecture used during training
        m1 = keras.Sequential([
            keras.layers.GRU(units=24, dropout=0.1, input_shape=(30, 26),
        ...
        m1(np.zeros((1, 30, 26), dtype=np.float32))
        # Load weights by name — Keras handles version differences automatically
        try:
            m1.load_weights(args.model, by_name=True, skip_mismatch=True)
            print("✓ Weights loaded via by_name=True")
        except Exception as w_err:
            print(f"by_name load failed ({w_err}). Trying direct weight load...")
            try:
                m1.load_weights(args.model)
                print("✓ Weights loaded directly")
            except Exception as w_err2:
                print(f"WARNING: Could not load weights ({w_err2}). Model uses random weights!")
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Frame", frame)          # show live window
        # cv2.imwrite(f"videos/image_{FRAME_ID:05d}.png", frame)  # disabled
        human_pose_extractor.roi.update(human_pose_extractor.keypoints_pixels_frame)

        k = cv2.waitKey(30)   # ~30fps playback
        if k == 27:
            break

    # Hold last frame open until ESC
    print("Video ended. Press ESC to close.")
    while True:
